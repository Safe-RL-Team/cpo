...
from own_utils.constraint_functions import *
import pdb
...
def constraint_cost(state, action):
    ...
    if args.env_name == 'CartPole-v1':
        costs = CartPole_go_left(state, dtype, device, bound=0.5)
    if args.env_name == 'LunarLander-v2':
        costs = LunarLander_angle(state, dtype, device)
...
def main_loop():
    ...
    """Here in each iteration trajectories are sampled from the current policy and the cpo_step method is called.
       In cpo_step the CPO update is implemented as described in Achiam et al."""
    ...
    for i_iter in range(args.max_iter_num):
        ...
        if log['env_avg_reward'] >= best_avg_training_reward:
            ...
            best_avg_training_reward = log['env_avg_reward']
