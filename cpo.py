...
def CGm(Avp_f, b, nsteps, rdotr_tol=1e-10):
    ...
    #print(rdotr.item())
    return x, math.sqrt(rdotr)


def cpo_step(env_name, policy_net, states, actions, advantages, cost_advantages, constraint_value,
             d_k, max_kl, damping, l2_reg, use_fim=True):
    ...
    # H: Hessian of kl-divergence
    # g: gradient of the objective
    # b: gradient of the constraint function
    # cc: current constraint value
    # max_kl: delta, defines the size of the trust region

    # Obtain objective gradient and step direction (g, (H^-1)*g)
    loss = get_loss()
    ...
    stepdir, r1 = CGm(Fvp, -loss_grad, its)  # (H^-1)*g
    if grad_norm == True:
        stepdir = stepdir / torch.norm(stepdir)

    # Obtain constraint gradient and step direction (b, (H^-1)*b)
    ...
    cost_stepdir, r2 = CGm(Fvp, -cost_loss_grad, its)  # (H^-1)*b
    cost_stepdir = cost_stepdir / torch.norm(cost_stepdir) # normalize cost_stepdir


    # constaint value - limit of constraint?
    d_k = tensor(d_k).to(constraint_value.dtype).to(constraint_value.device)
    cc = constraint_value - d_k #c

    # Define q, r, s where q = g^T.H^-1.g
    ...
    print('cc', cc.item(), ',', ((cc ** 2) / s - max_kl).item())
    if ((cc ** 2) / s - max_kl) > 0 and cc > 0:
        print('INFEASIBLE !!!!')
        opt_stepdir = -torch.sqrt(2 * max_kl / s).unsqueeze(-1) * Fvp(cost_stepdir) #cost_stepdir
    else:
    ...
    #new_params = prev_params + fullstep
    ...
    return loss, cost_loss, cc.item(), r1, r2